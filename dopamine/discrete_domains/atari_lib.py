...
def implicit_quantile_network(num_actions, quantile_embedding_dim, network_type, state,
                              num_quantiles):
    ...
    state_net = tf.contrib.slim.conv2d(state_net,
                                       32, [8, 8],
                                       stride=4,
                                       weights_initializer=weights_initializer)
    state_net = tf.contrib.slim.conv2d(state_net,
    ...
    quantiles = tf.random_uniform(quantiles_shape, minval=0, maxval=1, dtype=tf.float32)
    ...
    quantile_net = tf.contrib.slim.fully_connected(quantile_net,
                                                   state_net_size,
                                                   weights_initializer=weights_initializer)
    # Hadamard product.
    ...
    net = tf.contrib.slim.fully_connected(net, 512, weights_initializer=weights_initializer)
    quantile_values = tf.contrib.slim.fully_connected(net,
                                                      num_actions,
                                                      activation_fn=None,
                                                      weights_initializer=weights_initializer)
...
class AtariPreprocessing(object):
    ...
    def _pool_and_resize(self):
        ...
        transformed_image = cv2.resize(self.screen_buffer[0], (self.screen_size, self.screen_size),
